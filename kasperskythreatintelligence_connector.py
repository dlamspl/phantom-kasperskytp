...
class KasperskyThreatIntelligenceConnector(BaseConnector):
    ...
    def _extract_kaspersky_summary(self, response):
        ...
        if 'FileGeneralInfo' in response:
            if len(response['DetectionsInfo']) > 0:
                summary['categories'] = response['DetectionsInfo'][0]['DetectionName']
    ...
    def _process_response(self, r, action_result):
        ...
        # Always process a json response
    ...
    def _handle_test_connectivity(self, param):
        ...
        # make rest call
        ret_val, response = self._make_rest_call('/api/domain/example.com?sections=Zone', action_result, params=None, headers=None)
    ...
    def initialize(self):
        ...
        # self._pem_key = config.get('PEM key')
        self._tmp_pem_file = tempfile.NamedTemporaryFile(mode='w+b', delete=False)
        ...
        # On init we have to read the client cert and write it to file
        # with open('/tmp/key.pem', 'w') as pem_file:
        #    pem_file.write(config.get('PEM key'))
